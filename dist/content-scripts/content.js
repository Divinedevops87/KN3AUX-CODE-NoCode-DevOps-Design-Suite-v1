...
class GovernmentResourceAssistant {
    ...
    detectSiteType() {
        ...
        this.isGovernmentSite = governmentDomains.some(domain => hostname.includes(domain) || hostname.endsWith(domain));
    }
    injectBanner() {
    ...
    togglePanel() {
        const content = this.assistantPanel?.querySelector('#helperContent');
        const toggle = this.assistantPanel?.querySelector('#togglePanel');
        if (content && toggle) {
    ...
    simplifyForms() {
        ...
        formInputs.forEach(input => {
            ...
            if (label && !label.querySelector('.simple-help')) {
                const helpText = this.getSimplifiedHelp(input);
                if (helpText) {
